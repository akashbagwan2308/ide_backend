...
// ==========================================
// 3. SECURE SIMULATION (ICARUS VERILOG)
// ==========================================
app.post('/run', authenticateToken, (req, res) => {
    ...
    exec(`iverilog -g2012 -o ${outPath} ${filePath}`, { timeout: 10000, cwd: runDir }, (compileErr, compileStdout, compileStderr) => {
        ...
        exec(`vvp ${outPath}`, { timeout: 10000, cwd: runDir }, (runErr, runStdout, runStderr) => {
            ...
            if (runErr) return res.json({ status: "error", output: runStderr || runErr.message });

            // Returning raw VCD for the waveform viewer, and parsed JSON for the logic animator
...
app.listen(PORT, () => {
    console.log(`Unified LogicSilicon Backend (Icarus + Yosys) running on port ${PORT}`);
});